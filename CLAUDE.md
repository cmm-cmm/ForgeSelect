...
This is a zero-runtime-dependency, framework-agnostic TypeScript select/combobox component (a Select2 replacement). `src/ForgeSelect.ts` is the component itself; the rest of `src/` is small stateless helper modules it composes:

- **`src/ForgeSelect.ts`** (~2100 lines) — the whole component. One class that owns DOM construction, event binding, selection state, search/filtering, keyboard nav, virtual scrolling, and AJAX loading. There's no sub-component split; everything lives on `ForgeSelect` as private methods grouped by comment banners (`DOM setup`, `selection`, `rendering`, `remote data`).
...
- **`src/search.ts`** — `SearchIndex`, the token-search/relevance-scoring engine behind `filterOption`/`minSearchLength`/accent-insensitive search.
- **`src/selection.ts`** — tree-select helpers (`collectDescendantValues`, `computeCheckState`, `syncTreeAncestors`) plus `isGroup`/`findOption`/`collectValues`/`arraysEqual`, shared by selection and tree-checkbox logic.
- **`src/native-select.ts`** — `parseNativeOptions()`, deriving the `DataItem[]` shape from a mounted `<select>`'s `<option>`/`<optgroup>` children.
- **`src/remote.ts`** — small AJAX request-building/response-normalizing helpers used by `loadRemote()`.
- **`src/remote-cache.ts`** — `RemoteCache`, the TTL + FIFO-bounded response cache keyed by `remoteCacheKey()` (search text + page/cursor).
- **`src/option-renderer.ts`** — `renderOptionContent()`, the XSS-safe built-in row/value renderer (see "Rendering safety" below).
- **`src/dropdown-position.ts`** — `computeDropdownPlacement()`, pure drop-up/drop-down geometry shared by inline and portalled positioning.
...
- **Virtual scrolling**: kicks in automatically once `rows.length > VIRTUAL_THRESHOLD` (100), unless `virtualScroll: false`. `renderRows()` slices the visible window plus a `VIRTUAL_BUFFER` and pads with spacer `<li>`s to preserve scrollbar size. Rendered option content is memoized in `rowContentCache` (keyed by the `Option` object itself, not its value — `duplicateValuePolicy` only warns by default, so two options can share a value while carrying different labels — FIFO-evicted at `ROW_CACHE_LIMIT`) and cloned per row so templates don't re-run every scroll frame — state (selected/highlighted/disabled) is applied to the `<li>` wrapper, not baked into the cached content. Recycled `<li>` elements are a separate cache (`rowElementCache`, keyed by `rowElementKey()` — option value/group label only, no row index, so a narrowing search still hits) and must both be dropped together via `clearRowCaches()` whenever `data` is replaced, or a reused row can carry stale content/positional state from its previous occupant.
...
- **Rendering safety**: the built-in row/value renderer (`renderOptionContent()` in `src/option-renderer.ts`) builds DOM via `textContent`/`createElement`, not `innerHTML`, so avatar/label/description fields are XSS-safe by construction. A custom `templateResult`/`templateSelection` returning a string _is_ inserted via `innerHTML` — that's the one place caller-supplied HTML is trusted as-is, unless the caller also supplies `sanitizeTemplate` to run that string through their own sanitizer first.
